...
By default, the generator adds extra mineable resource zones to sectors for better gameplay balance:

- **Sectors WITH resources** (rich sectors): 1 extra zone added for variety
- **Sectors WITHOUT resources** (poor sectors): 2 extra zones added to encourage traffic spread

These extras are **replicated from existing zone macros** and placed at the sector's outer radius (protected by clamp radius). They are **not generated by the game at runtime** — their resource type is static and determined by the zone macro they reference (iron, ice, helium, hydrogen, methane, asteroid, or generic if profiling was unavailable).

This gives players more incentive to explore and spread mining operations away from the sector center, justifying the distance increase from the mod.

You can tune the density in `lib/config.sh`:
- `EXTRA_RESOURCE_ANCHORS_WITH_RESOURCE` / `EXTRA_RESOURCE_ANCHORS_NO_RESOURCE`: how many sector connections are selected as base points
- `EXTRA_RESOURCE_COUNT_WITH_RESOURCE` / `EXTRA_RESOURCE_COUNT_NO_RESOURCE`: how many variants per anchor
...
## Impact & Tuning Examples

### Generation Results (Factor 3x, Default Config)

With the default configuration (1 anchor × 1 extra for rich sectors, 1 anchor × 2 extras for poor sectors):

**Vanilla (Base Game):**
- Total sectors: 152
- Sectors with resources: 109 (~72%)
- Sectors without resources: 43 (~28%)
- Extra zones generated: ~105 total
- Distribution: Iron, Ice, Hydrogen, Helium, Methane, Asteroids

**DLC (7 DLCs Combined):**
- Total DLC sectors: 100+
- Extra zones generated: ~142 total (many profiled as `no_resource_profile`)
- Coverage: All 7 DLCs supported

**Overall:**
- Total extra zones across vanilla + DLC: ~247
- Sectors receiving extras: ~146 total
- Density: 1-2 extras per affected sector
- Distribution: Balanced between rich and poor sectors

### Tuning Examples

To adjust the number of extras, modify `lib/config.sh`:

**Example 1: More aggressive (3x extras)**
```bash
EXTRA_RESOURCE_ANCHORS_WITH_RESOURCE=2    # Was 1
EXTRA_RESOURCE_ANCHORS_NO_RESOURCE=1      # Stay 1
EXTRA_RESOURCE_COUNT_WITH_RESOURCE=2      # Was 1
EXTRA_RESOURCE_COUNT_NO_RESOURCE=3        # Was 2
# Result: ~500+ total extra zones (very dense)
```

**Example 2: Minimal (half the extras)**
```bash
EXTRA_RESOURCE_ANCHORS_WITH_RESOURCE=1    # Stay 1
EXTRA_RESOURCE_ANCHORS_NO_RESOURCE=1      # Stay 1
EXTRA_RESOURCE_COUNT_WITH_RESOURCE=0      # Was 1 (disabled)
EXTRA_RESOURCE_COUNT_NO_RESOURCE=1        # Was 2
# Result: ~100 total extra zones (very sparse)
```

**Example 3: DLC-only (focus on extending DLC)**
```bash
# Generate normally, but only enable for DLC sectors:
# Add DLC sector macros to EXCLUDE_SECTORS in reverse to include only DLC
# Or: Run generation twice, once for vanilla (save), once for DLC
```
...
- Extra resource zones are static (macro-driven, not generated at runtime)
- No performance impact in-game (changes are manifest before startup)

## FAQ: Extra Resource Zones

**Q: I don't see `_resourceextra_*` zones in my output**

A: Check that:
1. `generate.sh` completed with `SUCCESS - All XML diff patches generated` message
2. Your `_default/maps/xu_ep2_universe/zones.xml` exists
3. Your `_default/maps/xu_ep2_universe/clusters.xml` exists
4. You didn't exclude the sector with `EXCLUDE_SECTORS` in config.sh

Verify extras were generated:
```bash
grep -c "_resourceextra_" maps/xu_ep2_universe/sectors.xml
# Should return a number > 100
```

**Q: Why doesn't sector X have extras?**

Possible reasons:
1. **It's hazardous:** Listed in `EXCLUDE_SECTORS` (Void, Avarice, Sanctuary of Darkness, Terran Torus, etc.)
2. **No valid zones:** Sector contains only gates/SHCon (no regular zones)
3. **DLC profiling uncertainty:** `no_resource_profile` sectors may have lower priority (acceptable, macros are valid)
4. **Already has gates/accelerators:** These are protected and don't get resource extras

Check if a sector is excluded:
```bash
grep "Cluster_XX_Sector_YY" lib/config.sh
```

**Q: Can I mix `--no-highways` with default mode output?**

A: Not recommended. Generate all files with the **same mode** for consistency.

If you need both:
1. Generate default mode: `bash generate.sh 3` → save output
2. Clear output: `rm -rf maps extensions libraries`
3. Generate --no-highways mode: `bash generate.sh 3 --no-highways` → save separately

**Q: I enabled the mod but don't see changes in-game**

A: This is normal! The mod only generates diffs, which are XML patches. Make sure:
1. The mod is **enabled** in X4 launcher
2. You're in a **new game** (existing saves won't see changes)
3. You regenerated with your extracted files (if you customized anything)

**Q: Can I adjust the number of extras without editing code?**

A: Yes! Edit `lib/config.sh`:
```bash
# More extras:
EXTRA_RESOURCE_COUNT_WITHOUT_RESOURCE=4  # Default is 2

# Fewer extras:
EXTRA_RESOURCE_COUNT_WITHOUT_RESOURCE=1  # Default is 2
